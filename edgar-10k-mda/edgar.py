...
def download_10k_from(filename):
...
def extract_mda_para(form10k_file):
    ...
    # Find MDA section
    mda, end = parse_mda_item(text, 7)
        
    # Parse second time if first parse results in index
    if mda and len(mda.encode('utf-8')) < 1000:
        mda, _ = parse_mda_item(text, 7, start=end)

    if mda:
        filename = os.path.basename(form10k_file)
        name, ext = os.path.splitext(filename)
        mda_path = os.path.join(mda_dir + "_item7", name + "_item7.txt")
    ...
    # Find item1 section
    mda, end = parse_mda_item(text, 1)
        
    # Parse second time if first parse results in index
    if mda and len(mda.encode('utf-8')) < 1000:
        mda, _ = parse_mda_item(text, 1, start=end)

    if mda:
        filename = os.path.basename(form10k_file)
        name, ext = os.path.splitext(filename)
        mda_path = os.path.join(mda_dir + "_item1", name + "_item1.txt")
        print("writing item1 to {}".format(mda_path))
        with open(mda_path, 'w') as fout:
            fout.write(mda)
    else:
        print("extract item1 failed - {}".format(form10k_file))
...
def extract_mda(opt):
    ...
    global mda_dir
    mda_dir = opt["--mda-dir"]
    if not os.path.exists(mda_dir + "_item1"):
        os.makedirs(mda_dir + "_item1")
    if not os.path.exists(mda_dir + "_item7"):
...
def parse_mda_item(text, item, start=0):
    debug = False
    ...
    item7_ends = ['\nITEM ' + str(item) + 'A']
    if start != 0:
        item7_ends.append('\nITEM ' + str(item))  # Case: ITEM 7A does not exist
    item8_begins = ['\nITEM ' + str(item + 1)]
    """
    ...
    # Parse second time if first parse results in index
    #if mda and len(mda.encode('utf-8')) < 200:
    #    return parse_mda_item(text, item, start=end)
